...
fetch_other_pci_info() {
    ...
    while IFS= read -r line || [[ -n "$line" ]]; do
        # End of block (empty line or EOF)
        if [[ -z "$line" ]]; then
            if [[ -n "${block[Slot]:-}" ]]; then
                slot="${block[Slot]}"
                class="${block[Class]:-}"
                vendor="${block[Vendor]:-}"
                device="${block[Device]:-}"
                kernel_driver="${block[Driver]:-}"

                # Filter out GPU/NIC/Storage
                class_code=$(lspci -n | awk "/$slot/ {print \$2}" | cut -d: -f1)
                if [[ "$class_code" =~ ^01|^02|^03 ]]; then
                    block=()
                    continue
                fi

                # Sysfs info
                sys="/sys/bus/pci/devices/$slot"
                [[ -f "$sys/current_link_speed" ]] && speed=$(<"$sys/current_link_speed") || speed="unknown"
                [[ -f "$sys/current_link_width" ]] && width=$(<"$sys/current_link_width") || width="unknown"
                [[ -L "$sys/driver" ]] && driver_bound=$(basename "$(readlink "$sys/driver")") || driver_bound="N/A"
                [[ -f "$sys/numa_node" ]] && numa=$(<"$sys/numa_node") || numa="unknown"

                # Physical slot
                phys_slot=""
                if command -v dmidecode &>/dev/null; then
                    phys_slot=$(dmidecode -t slot 2>/dev/null | awk -v addr="$slot" '
                        /Designation/ {d=$2}
                        /Bus Address/ && $3==addr {print d}
                    ')
                fi

                # Human-readable
                echo -e "PCI Slot: $slot\nClass: $class\nVendor: $vendor\nDevice: $device\nKernel Driver: ${kernel_driver:-N/A}\nDriver Bound: ${driver_bound}\nPCIe Speed: $speed\nPCIe Width: $width\nNUMA Node: $numa\nPhysical Slot: ${phys_slot:-N/A}\n"

                # JSON
                other_pci_json=$(echo "$other_pci_json" | jq --arg slot "$slot" \
                                                            --arg class "$class" \
                                                            --arg vendor "$vendor" \
                                                            --arg device "$device" \
                                                            --arg kernel_driver "$kernel_driver" \
                                                            --arg driver_bound "$driver_bound" \
                                                            --arg speed "$speed" \
                                                            --arg width "$width" \
                                                            --arg numa "$numa" \
                                                            --arg phys_slot "$phys_slot" \
                                                            '. += [{
                                                                slot: $slot,
                                                                class: $class,
                                                                vendor: $vendor,
...
# ==========================
fetch_interconnect_info() {
    header "7. High-Speed Interconnects (NIC / DPU / InfiniBand / NVLink / NVSwitch)"
    separator
    local ic_json='{"ethernet":[],"infiniband":[],"nvswitch":[]}'
