...
# --- New Function: fetch_os_info ---
fetch_os_info() {
  header "0. Operating System Information"
  separator
  ...
  system_info_json=$(jq -n \
    --arg os_name "$os_name" --arg kernel_version "$kernel_version" --arg uptime "$uptime" \
    '{ os: $os_name, kernel: $kernel_version, uptime: $uptime }')
}
